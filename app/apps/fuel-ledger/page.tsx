...
export default function FuelLedgerPage() {
  return <main className="fuel-page">
    <header className="fuel-nav"><a href="/" className="fuel-brand" aria-label="MindVector home"><img className="fuel-logo" src="/fuel-ledger-mark.svg" alt="" />FuelLedger</a><nav aria-label="FuelLedger navigation"><a href="#product">Product</a><a href="#operations">Operations</a><a href="#intelligence">Intelligence</a><a href="#pricing">Pricing</a></nav><a className="fuel-nav-cta" href="https://fuelledger.mindvector.tech/login">Sign in <span>↗</span></a></header>
    ...
    <section className="fuel-pricing" id="pricing"><div className="pricing-heading"><p className="fuel-eyebrow"><span /> One complete plan</p><h2>FuelLedger<br /><em>Lifetime access.</em></h2><p>Everything an owner needs to run a petrol pump with clarity and control—without a recurring subscription.</p></div><article className="pricing-card"><div className="pricing-card-head"><div><img className="fuel-logo" src="/fuel-ledger-mark.svg" alt="" /><p>FUELLEDGER COMPLETE</p></div><a href="https://fuelledger.mindvector.tech/login">Start FuelLedger <b>↗</b></a></div><div className="lifetime-price"><p>Lifetime access <b>One-time payment</b></p><strong>₹24,000</strong><span>per petrol pump</span><small>Own the complete FuelLedger operating system with no recurring subscription.</small></div><p className="feature-intro">Every FuelLedger Complete plan includes:</p><ul className="feature-list">{features.map((feature) => <li key={feature}><span>✓</span>{feature}</li>)}</ul><a className="pricing-contact" href="https://fuelledger.mindvector.tech/login">Start FuelLedger <span>↗</span></a><p className="price-note">Prices exclude applicable GST. No transaction fee is charged by FuelLedger.</p></article></section>
